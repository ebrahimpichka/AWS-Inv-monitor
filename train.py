...
def test(model, test_loader, criterion, device, hook, print_log=False):
    '''
...
def train(model, train_loader, val_loader, criterion, optimizer, device, epochs, hook):
    ...
    for epoch in range(epochs):
        ...
        model.eval()
        hook.set_mode(smd.modes.EVAL) # setting the debugger hook mode to EVAL
        val_loss, val_acc = test(model, val_loader, criterion, device, hook, print_log=False)
        print(
...
def main(args):
    ...
    test(model, val_loader, loss_criterion, device, hook,print_log=True)
